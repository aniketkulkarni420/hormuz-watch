// Cloudflare Pages Function — reads aircraft_state from KV.
// Populated by .github/workflows/aircraft-scraper.yml (OpenSky ADS-B, every 15 min).
export async function onRequestGet({ env }) {
  if (!env.OIL_KV) return json({ error: "KV binding missing" }, 500);
  try {
    const raw = await env.OIL_KV.get("aircraft_state");
