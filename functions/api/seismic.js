// Cloudflare Pages Function — reads seismic_state from KV.
// Populated by .github/workflows/seismic-scraper.yml (USGS, hourly).
export async function onRequestGet({ env }) {
  if (!env.OIL_KV) return json({ error: "KV binding missing" }, 500);
  try {
    const raw = await env.OIL_KV.get("seismic_state");
