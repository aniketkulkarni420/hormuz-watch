// Cloudflare Pages Function — reads gdelt_state from KV.
// Populated by .github/workflows/gdelt-scraper.yml (GDELT 2.0, hourly).
export async function onRequestGet({ env }) {
  if (!env.OIL_KV) return json({ error: "KV binding missing" }, 500);
  try {
    const raw = await env.OIL_KV.get("gdelt_state");
